...
def main(argv):
    ...
    if len(DIRECTORY) < 1 or len(PROJECT) < 1: # 필수항목 값이 비어있다면
        print(FILE_NAME, "-d && -p options are mandatory") # 필수임을 출력
        sys.exit(2)
    result_path = DIRECTORY + "/result"
    if not os.path.exists(result_path):
        os.mkdir(result_path)
    pass_num, total = run(DIRECTORY, BENCHMARK, PROJECT, IDX, [], ASSERT)
    print("Passed : ", pass_num, "/", total)

    with open("/output"+("/total.result" if BENCHMARK != 'bugsinpy' else "/bugsinpy.result"), 'w') as f:
        f.write('+' + '-'*67 + '+\n')
    ...
    try:
        ...
        for project, pytest_info in pytest_json.items() :
            ...
            try :
                ...
                asts, files_src = ast.get_asts(DIRECTORY)
